...
class Transaction extends React.Component {
  ...
  searching = (e) => {
    ...
    this.setState({
      dataSearch: this.state.data.filter(
        (item) =>
          item.nama.toLowerCase().indexOf(e.target.value.toLowerCase()) > -1
      ),
    });
  };

  getBarang() {
      Services.getCoreBarang().then(res=>{
        this.setState({ barang:res.data.data })
      })
  }

  render() {
    const { barang, _pinjam_ } = this.state

    return (
      <div className="transaksi">
        <div className="d-flex justify-content-between mb-4">
          <button onClick={()=> this.setState({ _pinjam_: !_pinjam_ }) } className="btn btn-primary">Tambah Transaksi</button>
              ...
              <Card>
                ...
                >
                  <div className="title">
                    <h5>{item.nama}</h5>
                    <p className="m-0">
                      02 Desember 2021 &#9679; 03 Desember 2021
                    </p>
                  </div>
                  ...
                  <div className="d-flex status">
                    <div className="my-auto">
                      <h5
                        className={
                          "m-0 " +
                          (item.status_pinjam == "kembali"
                            ? "text-success "
                            : "") +
                          (item.status_pinjam == "hilang"
                            ? "text-danger "
                            : "") +
                          (item.status_pinjam == "masih pinjam"
                            ? "text-primary"
                            : "")
                        }
                      >
